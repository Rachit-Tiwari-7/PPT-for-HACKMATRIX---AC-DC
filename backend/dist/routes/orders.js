...
router.post("/create-payment", async (req, res) => {
    try {
        ...
        const order = await razorpayService_1.default.createOrder({
            amount,
            currency,
            receipt,
            notes: {
        ...
        res.json({
            success: true,
            order,
            keyId: razorpayService_1.default.getTestCredentials().keyId,
            testMode: true
        });
    }
    catch (error) {
...
router.post("/verify-payment", async (req, res) => {
    try {
        ...
        const isValid = await razorpayService_1.default.verifyPayment(paymentId, orderId, signature);
        if (!isValid) {
            return res.status(400).json({ success: false, error: "Invalid payment signature" });
        }
        const { data, error } = await db_1.default.from('orders').insert([
            {
                order_id: orderId,
...
router.get("/payment/:paymentId", async (req, res) => {
    try {
        ...
        const payment = await razorpayService_1.default.fetchPayment(paymentId);
...
router.get("/", async (req, res) => {
    try {
        const { data, error } = await db_1.default.from('orders').select('*').order('created_at', { ascending: false });
        if (error)
            throw error;
...
router.get("/user/:email", async (req, res) => {
    try {
        ...
        const { data, error } = await db_1.default.from('orders').select('*').eq('customer_email', email).order('created_at', { ascending: false });
        if (error)
            throw error;
        res.json({ success: true, orders: data || [], count: data?.length || 0 });
    }
...
router.get("/:orderId", async (req, res) => {
    try {
        ...
        const { data, error } = await db_1.default.from('orders').select('*').eq('order_id', orderId);
        if (error)
            throw error;
...
router.patch("/:orderId", async (req, res) => {
    try {
        ...
        const { data, error } = await db_1.default.from('orders').update({ status, shipping_address, updated_at: new Date().toISOString() }).eq('order_id', orderId).select();
        if (error)
            throw error;
        if (!data || data.length === 0)
            return res.status(404).json({ success: false, error: 'Order not found' });
